...
export class AuthService {
  ...
  async signIn(incomingLogin: string, password: string) {
    try {
      ...
      return {
        access_token: await this.jwtService.signAsync({ id: user.id, login: user.login }),
      };
    } catch (error) {
  ...
  async resetPassword(resetPasswordDto: ResetPasswordDto) {
    try {
      ...
      const updatedUser = await this.userService.updateUser(user, resetPasswordDto)

      return {
        access_token: await this.jwtService.signAsync({ id: updatedUser.id, login: updatedUser.login }),
      }
    } catch (error) {
